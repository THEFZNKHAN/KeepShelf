...
function check(): void {
  const parsed = parseGoodreadsPage(document);
...
async function handleSave(): Promise<void> {
  if (!currentParsed) {
    showToast("Book details not loaded yet. Wait for the page.", "error");
    return;
  }

  const bookId = goodreadsBookIdFromUrl(location.href);
  let response;
  try {
    response = await sendToBackground({
      ...
      item: {
        ...currentParsed,
        googleQuery: bookId ?? currentParsed.title,
