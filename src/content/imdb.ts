...
function check(): void {
  const parsed = parseImdbPage(document);
...
async function handleSave(): Promise<void> {
  if (!currentParsed) {
    showToast("Title details not loaded yet. Wait for the page.", "error");
    return;
  }

  const imdbId = imdbIdFromUrl(location.href);
  let response;
  try {
    response = await sendToBackground({
      ...
      item: {
        ...currentParsed,
        googleQuery: imdbId ?? currentParsed.title,
