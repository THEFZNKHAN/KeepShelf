...
export type KeepTarget = "media" | "books";
...
export function getKeepTargetForType(
  ...
  if (type === "book") {
    ...
    return {
      target: "books",
      noteId: settings.booksNoteId,
      noteUrl: settings.booksNoteUrl,
    };
  }
  ...
  return {
    target: "media",
    noteId: settings.mediaNoteId,
    noteUrl: settings.mediaNoteUrl,
  };
}
...
export function missingKeepConfigMessage(target: KeepTarget): string {
  return target === "books"
    ? "Link your Books Keep note in Settings."
    : "Link your Media Keep note in Settings.";
}
